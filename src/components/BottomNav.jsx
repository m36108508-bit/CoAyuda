      ...
      <div className="mx-auto grid max-w-2xl grid-cols-3 text-center">
        {ITEMS.map(item => {
          ...
          return (
            ...
            >
              <span className={`flex h-8 w-8 items-center justify-center rounded-xl text-lg ${active ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
                {item.icon}
              </span>
              {item.label}
            </button>
